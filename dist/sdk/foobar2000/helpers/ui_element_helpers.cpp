...
void ui_element_helpers::ui_element_edit_tools::standard_edit_context_menu(LPARAM p_point,ui_element_instance_ptr p_item,unsigned p_id,HWND p_parent) {
	...
	if (sourceItemEmpty) {
		...
		WIN32_OP_D( menu.AppendMenu(MF_STRING,ID_REPLACE,pfc::stringcvt::string_os_from_utf8(ReplaceUIElementCommand)) );
	}
	WIN32_OP_D( menu.AppendMenu(MF_SEPARATOR,(UINT_PTR)0,TEXT("")) );
	...
	WIN32_OP_D( menu.AppendMenu(MF_STRING | (api->is_paste_available() ? 0 : (MF_DISABLED|MF_GRAYED)),ID_pfc::stringcvt::string_os_from_utf8(PasteUIElementCommand)) );
